...
def globalsetting(request):
    ...
    if request.method == "POST":
        ...
        scanner = request.FILES.get('scanner')
        ...
        if data is None:
            ...
            data = GlobalSettings(SiteName=name, SiteContact=contact, SiteAddress=address, SiteEmail=email,
                                  Sitetwitterlink=twitterlink,Sitefacebooklink=facebooklink,Sitelinkdinlink=linkdinlink,
                                  Siteinstagram=instagram, Siteyoutubelink=youtubelink,Whatsapp=Whatsapp)
        else:
