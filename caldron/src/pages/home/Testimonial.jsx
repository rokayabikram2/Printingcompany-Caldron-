...
const Testimonial = () => {
    ...
    const testiData = async () =>{
        try{
            ...
            const testimageData = response.data.filter(
                (item) => item.status === "Publish" && item.page_type === "Testimonial/Background"
            );
            setTestimage(testimageData[0]);
    ...
    return (
        <section className="md:py-20 py-16 relative bg-cover bg-fixed bg-no-repeat text-white" style={{backgroundImage: `url(${testimage && testimage.bannerimage})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover'}}>
