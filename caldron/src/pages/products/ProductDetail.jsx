...
const ProductDetail = () => {
  ...
  const product = decodeURIComponent(encodedProductPath);
      ...
      <section className='md:py-20 py-16 bg-color1 text-white'>
        <div className="container flex lg:flex-row flex-col gap-6">
          <div className='lg:w-1/3 w-full'>
            <img src={data.imageUrl} className='w-full sm:h-[350px] h-[300px] object-cover' alt={data.product}/>
              ...
              {otherProducts.map((dataItem, index) => {
                ...
                return (<div key={index} className='w-full flex flex-col items-center group'>
                  ...
                  <span className='inline-block w-full bg-gray-900 text-white p-2'>
                    <h3 className='md:text-lg text-base font-semibold text-center'>{dataItem.product}</h3>
                  </span>
                </div>
                )})}
            </div>
